...
The Neon documentation is the source of truth for all Neon-related information. Always verify claims against the official docs before responding. When official docs, CLI output, MCP output, and skill instructions disagree, prefer the official docs; if the docs cannot be fetched, say that verification failed rather than guessing. If docs retrieval, CLI commands, or MCP commands fail, do not guess. Explain the failure, provide the best verified answer available, and ask the user whether to retry with another method. If verification cannot be completed, state that explicitly, provide only non-verified guidance with a clear caveat, and ask the user to retry or provide the missing context.
...
If the user provides an incomplete or invalid Neon connection string, project name, or runtime constraint, ask one concise clarification question before running setup commands.
...
If the user does not want to use `neonctl init`, or if `neonctl init` fails, use the fallback path below only when the user is using Cursor/VS Code, an MCP-capable agent, or wants the agent-skill workflow. Do not mix these paths.
...
1. Inspect the environment and existing Neon setup.
2. Choose or create a project.
3. Get the connection string and write it to `.env` as `DATABASE_URL` (create `.env` if missing; do not overwrite unrelated variables).
4. Choose the connection method and driver.

Only if the user asks for additional setup, run the optional branches in order:

- Auth: provision Neon Auth if needed, then configure it from the official docs.
- ORM: check for an existing ORM and ask before installing or generating configuration.
- Schema: inspect existing migrations or schema files and offer an example schema if needed.

**1. Select Organization and Project**

Use MCP server or CLI to list organizations and projects. If the user has not already chosen a project, ask one concise question: (1) use an existing Neon project, or (2) create a new Neon project. Only after the user chooses, run the corresponding Neon CLI or MCP command.

**2. Get Connection String**

Use MCP server or CLI to get the connection string. Store it in `.env` as `DATABASE_URL`. Read the file first before modifying to avoid overwriting existing values. If `.env` does not exist, create it with only `DATABASE_URL=...`. If the file cannot be written, tell the user the connection string and ask them to add it manually.
...
If browser authentication fails, times out, or the user cannot complete login, stop the setup and tell the user how to authenticate manually (for example, by running `neonctl auth` or using an API key) instead of continuing with guessed values.
